    ...
    <div class="panel">
        ...
        <form class="form-horizontal" action="#" id="preBookingform" method="POST">
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                    <div class="col-sm-4">
                       <label class="control-label" for="name">{{__('Mobile No* ')}}</label>
                    ...
                    <!-- <div class="col-sm-3">
                        <label class="control-label" for="name">{{__('Password*')}}</label>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        <label class="control-label" for="type">{{__('User Type')}}</label>
                        <select class="form-control demo-select2-placeholder" name="user_type" id="user_type" >
                    ...
                    <div class="col-sm-4">
                        ...
                        <select class="form-control demo-select2-placeholder" name="type" id="type" >
                                ...
                                <option value="C">{{__('Credit')}}</option>
                                <option value="D">{{__('Debit')}}</option>
                                <option value="P">{{__('Prepayment')}}</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control demo-select2-placeholder" name="institutes" id="Institutes">
        ...
        <?php
	    $allopen = \App\Order::where('invoicelookuptype','S')->where('payment_status','O')->where('invoice_number',  'not like', '%' . 'web' . '%')->where('invoicedate', '<=', date('d.m.Y',strtotime("-1 days")))->get();
        
        $allinv = array(); //print_r($allopen);
       // if(isset($_POST['invoice_type']) && $_POST['invoice_type'] == 'S' && count($allopen) > 0){
        if(count($allopen) > 0){
			echo "<p style='color:red'><strong>Open Invoice are:</strong> ";
			foreach($allopen as $inv):
                $allinv[] = $inv->invoice_number;
				echo "<a target='_blank' href='/admin/ARinvoice_header_workbench/view/".$inv->id."/".$inv->invoice_number."'>".$inv->invoice_number."</a>&nbsp;|&nbsp; ";
			endforeach;
			echo " </p>";
			//echo "<p style='color:red'><strong>Open Invoice are:</strong> ".implode(", ",$allinv)." </p>";
			}
        ?>
        <!--===================================================-->
        <!--End Horizontal Form-->
        ...
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-body">
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-styled">{{ __('Verify') }}</button>
                </div>
            </form>
        </div>
    </div>
...
$("#mobile1").autocomplete({
        ...
        source: '{{ url('admin/ARinvoice_header_workbench/CustomerForPrebooking')}}',
        focus: function( event, ui ) {
            console.log(ui);
                 ...
                 $( "#mobile1"  ).val( "xxxxxxx" + ui.item.phone.substring(7) );
                  ...
                  $("#customer_id").val(ui.item.cutomer_id);
...
function copyme(obj){
    ...
    $("#preBookingform").submit(function(e){
        e.preventDefault();
     $.post("{{ route('order.ARinvoiceStore') }}",$("#preBookingform").serialize()).done(function( data ) {
          ...
          // console.log(data);
           if(data.status == "newuser")
            ...
            if(data.status == "olduser"){
                ...
                $("#preBookingform")[0].reset(); 
               window.location.replace("{{ url('admin/ARinvoice_header_workbench/view')}}/" +orderid +'/'+newInvcNum);
     ...
     $('#Resend_code').click(function(e){
       ...
       // var country_code = $("input[name=country_code]").val();
        ...
        //console.log(phone);
        $.post( "{{ route('order.resend_code') }}",{ email: _email, phone: phone, _token: "{{ csrf_token() }}" }, function( data ) {
            //console.log(data);
            data = JSON.parse(data);
    ...
    $("#verify_phone_guest").submit(function(e){
        e.preventDefault();
     $.post("{{ route('order.verifyPhonePrebooking') }}",$( "#verify_phone_guest" ).serialize()).done(function( data ) {
            data = JSON.parse(data);
