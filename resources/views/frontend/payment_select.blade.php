                                        ...
                                        <div class="col-md-6 mx-auto">
                                            <div class="row">
                                                @if(\App\BusinessSetting::where('type', 'paypal_payment')->first()->value == 1)
                                                    <div class="col-6">
                                                        <label class="payment_option mb-4" data-toggle="tooltip" data-title="Paypal">
                                                            <input type="radio" id="" data-val="gateway" name="payment_option" value="paypal">
                                                            <span>
                                                ...
                                                @if(\App\BusinessSetting::where('type', 'stripe_payment')->first()->value == 1)
                                                    <div class="col-6">
                                                        <label class="payment_option mb-4" data-toggle="tooltip" data-title="Stripe">
                                                            <input type="radio" id="" data-val="gateway" name="payment_option" value="stripe">
                                                            <span>
                                                ...
                                                @if(\App\BusinessSetting::where('type', 'sslcommerz_payment')->first()->value == 1)
                                                    <div class="col-6">
                                                        <label class="payment_option mb-4" data-toggle="tooltip" data-title="sslcommerz">
                                                            <input type="radio" id="" data-val="gateway" name="payment_option" value="sslcommerz">
                                                            <span>
                                                ...
                                                @if(\App\BusinessSetting::where('type', 'instamojo_payment')->first()->value == 1)
                                                    <div class="col-6">
                                                        <label class="payment_option mb-4" data-toggle="tooltip" data-title="Instamojo">
                                                            <input type="radio" id="" data-val="gateway" name="payment_option" value="instamojo">
                                                            <span>
                                                ...
                                                @if(\App\BusinessSetting::where('type', 'razorpay')->first()->value == 1)
                                                    <div class="col-6">
                                                        <label class="payment_option mb-4" data-toggle="tooltip" data-title="Razorpay">
                                                            <input type="radio" id="" data-val="gateway" name="payment_option" value="razorpay">
                                                            <span>
                                                ...
                                                @if(\App\BusinessSetting::where('type', 'paystack')->first()->value == 1)
                                                    <div class="col-6">
                                                        <label class="payment_option mb-4" data-toggle="tooltip" data-title="Paystack">
                                                            <input type="radio" id="" data-val="gateway" name="payment_option" value="paystack">
                                                            <span>
                                                ...
                                                @if(\App\BusinessSetting::where('type', 'voguepay')->first()->value == 1)
                                                    <div class="col-6">
                                                        <label class="payment_option mb-4" data-toggle="tooltip" data-title="VoguePay">
                                                            <input type="radio" id="" data-val="gateway" name="payment_option" value="voguepay">
                                                            <span>
                                                ...
                                                @if(\App\Addon::where('unique_identifier', 'paytm')->first() != null && \App\Addon::where('unique_identifier', 'paytm')->first()->activated)
                                                    <div class="col-6">
                                                        <label class="payment_option mb-4" data-toggle="tooltip" data-title="Paytm">
                                                            <input type="radio" id="" data-val="gateway" name="payment_option" value="paytm">
                                                            <span>
                                                ...
                                                @if(\App\BusinessSetting::where('type', 'cash_payment')->first()->value == 1)
                                                    ...
                                                    @endphp
                                                    @if($digital != 1 && Session::has('is_cod_available'))
                                                        @if(Session::get('is_cod_available')== true)
                                                        <div class="col-6">
                                                            <label class="payment_option mb-4" data-toggle="tooltip" data-title="Cash on Delivery">
                                                                <input type="radio" data-val="cash" id="iscash_on_delivery" name="payment_option" value="cash_on_delivery">
                                                                <span>
                                                ...
                                                @if (Auth::check())
                                                    @if (\App\Addon::where('unique_identifier', 'offline_payment')->first() != null && \App\Addon::where('unique_identifier', 'offline_payment')->first()->activated)
                                                        @foreach(\App\ManualPaymentMethod::all() as $method)
                                                            @if(Session::get('shipping_type') == "pickup_point")
                                                                <div class="col-6">
                                                                    <label class="payment_option mb-4" data-toggle="tooltip" data-title="{{ $method->heading }}">
                                                                        <input type="radio" id="" class="manual" name="payment_option" value="manual_{{ $method->heading }}">
                                                                        <span>
                                                                    ...
                                                                    @if($method->type != "pay_on_counter")
                                                                        <div class="bankInfo bankInfo_{{ $method->id }}" style="display:none;"></div>
                                                                        @endif
                                                                <!-- Mdal -->
                                                                </div>
                                                            @else
...
@section('script')
    <script type="text/javascript">
        ...
        $('.bankInfo').hide();
/*
var iscash_of_delivery = '';
var codprice = '<?= session('codprice') ?>';
var cur = '<?= session('currencySymbol') ?>';
var newtotal = total = $('.total').attr('data-original');
var coupon_discount = parseFloat('<?= Session::get('coupon_discount') ?>');
*/
function checkPayment(payment_type){

    $(".bankInfo").hide();

    if(payment_type.indexOf('manual') > -1)
    {
        payment_type = payment_type.split("manual_");
        // alert(payment_type[1]);
        $.post( "{{ route('checkout.checkManual') }}",{ payment_type: payment_type[1],_token: "{{ csrf_token() }}" }, function( data ) {
            ...
            {   
                ...
                $('.bankInfo_'+data.id).html('');
                var html='';
                var bank_info = JSON.parse(data.bank_info);
                $.each(bank_info, function(index, value) {
                    html = '<table class="table table-striped" style="border: 1px solid antiquewhite;"><thead></thead><tbody><tr><th>Bank Name</th><th>Account Name</th><th>Account Number</th></tr><tr><td>'+value.bank_name+'</td><td>'+value.account_name+'</td><td>'+value.account_number+'</td></tr></tbody></table>';
                }); 
                
                
                $('.bankInfo_'+data.id).html(html);
                $('.bankInfo_'+data.id).slideDown();
            }
            else{
    ...
    $.post( "{{ route('checkout.checkforpayment') }}",{ payment_type: payment_type,_token: "{{ csrf_token() }}" }, function( data ) {               
...
$(document).ready(function(){
    ...
    if (document.getElementById('iscash_on_delivery') && document.getElementById('iscash_on_delivery').checked) {
    
    checkPayment('cash_on_delivery');
    }
});
