                                        ...
                                        <div class="col-md-6">
                                            <table class="details-table table">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{ single_price($order->orderDetails->sum('price') + $order->orderDetails->sum('tax') + $order->shippingprice + $order->codprice - $order->coupon_discount) }}</td>
                                                </tr>
                                                <tr>
                                                    <td class="w-50 strong-600">{{__('Shipping')}}:</td>
                                                    <td> 
                                                    @foreach($order->orderDetails as $orderDetails)
                                                        @if ($orderDetails->shipping_type != null && $orderDetails->shipping_type == 'home_delivery')
                                                            {{ __('Home Delivery') }}
                                                        @elseif ($orderDetails->shipping_type == 'pickup_point')
                                                            @if ($orderDetails->pickup_point != null)
                                                                {{ $orderDetails->pickup_point->name }} ({{ __('Pickup Point') }})
                                                            @else
                                                                {{ __('Pickup Point') }}
                                                            @endif
                                                        @endif
                                            ...
                                            <table class="table details-table">
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <th>{{__('Shipping')}}</th>
                                                        <td class="text-right">
                                                            <span class="text-italic">{{ single_price($order->shippingprice) }}</span>
                                                        </td>
                                                    </tr>
                                                    <?php if($order->codprice > 0){ ?>
                                                    <tr>
                                                        <th>{{__('COD Price')}}</th>
                                                        <td class="text-right">
                                                            <span class="text-italic">{{ single_price($order->codprice) }}</span>
                                                        </td>
                                                    </tr>
                                                    <?php } ?>
                                                    <tr>
