           ...
           $.ajax({
               ...
               success: function(data){
                  // console.log(data);
                    ...
                    $('.product-gal-img').html('');
                    $('.xzoom-thumbs').html('');
                  
                    var image = data.image;
                    var image_type = data.image_type;
                    if(image_type == "variant_image")
                    {
                        var image_url ="{{ url('/public') }}/"+data.image;
                        //$('.product-gal').html('<img src="'+image_url+'" class="xzoom img-fluid lazyloaded" data-src="'+image_url+'" xoriginal="'+image_url+'" style="width: 100%;"><div class="product-gal-img"></div><div class="product-gal-thumb"><div class="xzoom-thumbs"><a href="'+image_url+'"><img src="'+image_url+'" class="xzoom-gallery xactive lazyloaded" data-src="'+image_url+'" xpreview="'+image_url+'" width="80"></a></div>');
                        $('.product-gal-img').html('<img src="'+image_url+'" class="xzoom img-fluid lazyloaded" data-src="'+image_url+'" xoriginal="'+image_url+'" style="width: 100%;">');
                        $('.xzoom-thumbs').html('<a href="'+image_url+'"><img src="'+image_url+'" class="xzoom-gallery xactive lazyloaded" data-src="'+image_url+'" xpreview="'+image_url+'" width="80"></a>');
                        ...
                        $('.xzoom-thumbs').html('');
                      // $('.product-gal').html();
                      var image_con = JSON.parse(image);
                     // alert(image_con);
                       $.each(image_con, function(key, value){
                           //alert(value);
                             var image_url ="{{ url('/public') }}/"+value;
                            //alert(image_url);
                       
                        if(key == 0)
                        {
                            $('.product-gal-img').html('<img src="'+image_url+'" class="xzoom img-fluid ls-is-cached lazyloaded" data-src="'+image_url+'" xoriginal="'+image_url+'" style="width: 100%;">');
                            $('.xzoom-thumbs').append('<a href="'+image_url+'"><img src="'+image_url+'" class="xzoom-gallery xactive ls-is-cached lazyloaded" data-src="'+image_url+'" xpreview="'+image_url+'" width="80"></a>');
                        }
                        else{
                            $('.xzoom-thumbs').append('<a href="'+image_url+'"><img src="'+image_url+'" class="xzoom-gallery ls-is-cached lazyloaded" data-src="'+image_url+'" width="80"></a>');
                        }
                ...
                defaultScale: -1
                    ...
                    // update table data
                    console.log(data.bulk_id);

                    // reset table
                    $("#bulk_prod_info tbody").empty();
                    $.each(data.bulk_id, function(key, value)
                    {
                        var _tr = `<tr class="variation_row`+key+`">
                        <td class="options">
										<label title="item1">
                                        <input type="radio" id="" onClick="selectvarient('`+value.qty+`','`+value.price+`')" name="select" value="`+value.variant+`">
                                        <img />
										</label>
                                        </td>
                                        <td class="bulk_variation">`+value.variant+`</td>
                                        <td class="bulk_qty">`+value.qty+`</td>
                                        <td class="bulk_price">`+value.price+`</td>
                                        
                                    </tr>`;
                        $("#bulk_prod_info tbody").append(_tr);
                    });

                    if(data.bulk_id.length == 0)
                    {
                        $("#bulk_prod_info").hide();
                    }
                    else
