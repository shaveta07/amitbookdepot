                        ...
                        <div class="widget-balance py-3">
                            <div class="text-center">
                                <div class="heading-4 strong-700 mb-4">
                                    @php
                                    $filter_date = date('Y-m-d H:i:s', strtotime('-30 days'));
                                        $orderDetails = \App\OrderDetail::where('seller_id', Auth::user()->id)->where('created_at', '>=', $filter_date)->get();
                                        $total = 0;
                                ...
                                <table class="text-left mb-0 table w-75 m-auto">
                                    <tbody>
                                        ...
                                        <tr>
                                            @php
                                            $filter_date = date('Y-m-d H:i:s', strtotime('-30 days'));
                                            $filter_dates = date('Y-m-d H:i:s', strtotime('-60 days'));
                                                $orderDetails = \App\OrderDetail::where('seller_id', Auth::user()->id)->where('created_at', '>=', $filter_dates)->where('created_at', '<=', $filter_date)->get();
                                                $total = 0;
                                                foreach ($orderDetails as $key => $orderDetail) {
                                                    if($orderDetail->order != null && $orderDetail->order->payment_status == 'paid'){
                                                        $total += $orderDetail->price;
