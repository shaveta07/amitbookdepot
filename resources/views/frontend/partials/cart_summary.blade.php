            ...
            <tbody>
                ...
                @foreach (Session::get('cart') as $key => $cartItem)
                    @php
                        ...
                        if (\App\BusinessSetting::where('type', 'shipping_type')->first()->value == 'order_wise_shipping') {
        ...
        <table class="table-cart table-cart-review">
            ...
            <tfoot>
                <tr class="cart-subtotal">
                    <th>{{__('Subtotal')}}</th>
                    <td class="text-right">
                        <span class="strong-600" id="subtotal" data-val="{{ $subtotal }}">{{ single_price($subtotal) }}</span>
                    </td>
                </tr>

                <tr class="cart-shipping nxtcod">
                    <th>{{__('Tax')}}</th>
                    <td class="text-right">
                        <span class="text-italic" id="tax" data-val="{{ $tax }}">{{ single_price($tax) }}</span>
                    </td>
                </tr>
<?php
//dd(session()->all());
//echo session('codprice');

?>
                @if(session('show_cod'))
                @if(session('show_cod') == true)
                <tr class="cart-shipping" id="cash_on_delivery">
                    <th>{{__('Cash On Delivery')}}</th>
                    <td class="text-right">
                        <span class="text-italic" id="COD">
                            @if(Session::has('addcodprice'))
                                {{ single_price(session('addcodprice')) }}
                            @else
                                {{ single_price(session('addcodprice')) }}
                            @endif
                        </span>
...
<?php
$cod=0;  
if(Session::has('codprice')){
$cod = Session::get('addcodprice');	
	}
//echo Session::get('addcodprice');	
?>
<tr class="cart-shipping" id="cash_on_delivery" <?php if(Session::get('addcodprice') == 0){ ?> style="display:none" <?php } ?>>
                    <th>{{__('Cash On Delivery')}}</th>
                    <td class="text-right">
                        <span class="text-italic" id="COD">
                            @if(Session::has('addcodprice'))
                                {{ single_price(Session::get('addcodprice')) }}
                            @else
                                {{ Session::get('addcodprice') }}
                            @endif
                        </span>
               ...
               <?php
$allshipprice = 0;
if(Session::has('shipping_type') && Session::get('shipping_type') == 'pickup_point'){
	$shipping = 0;
	}
if(Session::get('final_shipping_price') > 0){
	$allshipprice = Session::get('final_shipping_price')+$shipping;
	}else{
		$allshipprice = $shipping;
	}
...
?> 
                <tr class="cart-shipping">
                    ...
                    <td class="text-right">
                        <span class="text-italic" id="shipping" data-val="{{ $allshipprice }}">
                            <?php 
                            
                                echo single_price($allshipprice); 
                ...
                @php
                    $total = $subtotal+$tax+$allshipprice+$cod;
                    if(Session::has('coupon_discount')){
                ...
                <tr class="cart-total">
                    ...
                    <td class="text-right">
                    <!-- <strong><span class="total" >{{ single_price($total) }}</span></strong> -->
                        <strong><span class="total" data-original="{{ $total }}">{{ single_price($total) }}</span></strong>
                    </td>
                </tr>
            </tfoot>
        </table>
