                    ...
                    <div class="col-lg-6">
                        ...
                        <div class="product-description-wrapper">
                            ...
                            <div class="row align-items-center">
                                ...
                                @endif
                                <div class="col-auto">
                                <small class="mr-2">{{__('author')}}: </small><br>
                                <?php $author= \App\Author::where('id', $detailedProduct->author_id)->first(); ?>
                                   @if($author) {{ $author->name }} @endif
                                </div>
                                <div class="col-auto">
                                <small class="mr-2">{{__('origin')}}: </small><br>
                                <?php $origin= \App\Product::where('id', $detailedProduct->id)->first(); ?>
                                   @if($author) {{ $origin->origin }} @endif
                                </div>
                            </div>
                            ...
                            <div class="row no-gutters mt-4">
                                    ...
                                    <div class="col-10">
                                        <div class="product-price-old">
                                           ...
                                           {{ format_price($detailedProduct->mrp) }}
                            ...
                            <?php
							//print_r($userData);
							//$bundleprices = json_decode($detailedProduct->bundleprice,true); ?>
                            <?php if(sizeof($detailedProduct->bulks))
                            {
                            echo "<table id='bulk_prod_info' class='table variation_table'><thead><tr><td>Select</td><td>Variant</td><td>Quantity</td><td>Price</td></tr></thead><tbody>"; 
                            
                            /*
                            foreach($detailedProduct->bulks as $key => $bulk){
                            if(Auth::guest()){
                            
                                    if($bulk->customertype == 'customer'){ ?>
                                    <tr class="variation_row" data-id='<?= $bulk->id ?>'>
                                        <td class="bulk_qty"><?= $bulk->qtyrange ?></td>
                                        <td class="bulk_price"><?= $bulk->overideprice ?></td>
                                        <td class="select_option"><a href="javascript:void(0)" class="selectvarient" onClick="selectvarient('<?= $bulk->qtyrange ?>','<?= $bulk->overideprice ?>')">Select</a></td>
                                    </tr>
                                    <?php
									}
									}else{
										if($bulk->customertype  == $userData->user_type) {?>
                                            <tr class="variation_row" data-id='<?= $bulk->id ?>'>
                                            <td class="bulk_qty"><?= $bulk->qtyrange ?></td>
                                             <td class="bulk_price"><?= $bulk->overideprice ?></td>
                                            <td class="select_option"><a href="javascript:void(0)" class="selectvarient" onClick="selectvarient('<?= $bulk->qtyrange ?>','<?= $bulk->overideprice ?>')">Select</a></td>
                                            </tr>
                                           <?php } elseif($bulk->customertype  == 'customer' && ($userData->user_type == 'customer' || $userData->user_type == '' )) { ?>
                                            <tr class="variation_row" data-id='<?= $bulk->id ?>'>
                                            <td class="bulk_qty"><?= $bulk->qtyrange ?></td>
                                             <td class="bulk_price"><?= $bulk->overideprice ?></td>
                                            <td class="select_option"><a href="javascript:void(0)" class="selectvarient" onClick="selectvarient('<?= $bulk->qtyrange ?>','<?= $bulk->overideprice ?>')">Select</a></td>
                                    ...
                                    <div class="col-10">
                                        <div class="product-quantity d-flex align-items-center">
                                            <div class="input-group input-group--style-2 pr-3" style="width: 160px;">
                                                ...
                                                <input type="hidden" id="isbundle" name="isbundle" class="isbundle" value="no" />
                                                <input type="hidden" id="bundleprice" name="bundleprice" class="bundleprice" value="0" />
                                                <input type="text" name="quantity" class="cartqty form-control input-number text-center" placeholder="1" value="1" min="1" max="10">
                                                <span class="input-group-btn">
                            ...
                            <div class="row no-gutters mt-3">
                                ...
                                <div class="col-10">
                                <p>View <a href='{{ route('products.getProductSeller', ['id' => $detailedProduct->id] ) }}'>More Sellers</a> starting from ₹380</p>
                                </div>
                            </div>
                            <div class="row no-gutters mt-3">
                                <div class="col-2">
                    ...
                    <div class="seller-info-box mb-3">
                        <div class="sold-by position-relative">
                            ...
                            @php
								...
								if(isset($detailedProduct->user->products)){
                ...
                <div class="col-xl-9">
                    <div class="product-desc-tab bg-white">
                        <div class="tabs tabs--style-2">
                            <ul class="nav nav-t    abs justify-content-center sticky-top bg-white">
                                <li class="nav-item">
...
@section('script')
    <script type="text/javascript">
    	...
    	function selectvarient(qty,price){
	$('.cartqty').val(qty);
   var p =parseFloat(price*qty);
	//var p = parseFloat(price/qty);
    $('.product-price-inner').html(p.toFixed(2));
	//var v = $('.product-price-old del').val();
	//var q = v*qty;
	//$('.product-price-old del').val(q);
    price=p.toFixed(2);
	$('strong#chosen_price').html('Rs'+price);
	$('#bundleprice').val(price);
	$('#isbundle').val('yes');
	}
