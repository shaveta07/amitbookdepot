    ...
    <div class="panel">
        ...
        <form class="form-horizontal" action="{{ route('categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                    <label class="col-sm-2 control-label" for="name">{{__('Istaxapplicable ')}}</label>
                    <div class="col-sm-10">
                        <select name="istaxapplicable" required class="form-control demo-select2-placeholder">
                            <option value="yes" @if ($category->istaxapplicable  == 'yes') selected @endif>{{__('Yes')}}</option>
                            <option value="no"  @if ($category->istaxapplicable  == 'no') selected @endif >{{__('NO')}}</option>
                        </select>
                    </div>
                </div>
                <div class="form-group">
