            ...
            <div class="col-sm-12">
                <fieldset class="scheduler-border">
                    ...
                    <div class="control-group">
                        <form method="get" class="form-inline" action="{{route('Task.search')}}">
                        <div class="form-group">
                            <label for="startdate">Start Date:</label>
                            <input style="line-height:20px;" type="date" name="startdate" value="<?php if($startdate != ''){echo $startdate; } ?>" class="form-control" id="startdate">
                        </div>
                        <div class="form-group">
                            <label for="enddate">End Date:</label>
                            <input style="line-height:20px;" type="date" name="enddate" value="<?php if($enddate != '') {echo $enddate; } ?>" class="form-control" id="enddate">
                        </div>
                        <div class="form-group">
...
<table class="table table-bordered table-striped table-vcenter js-dataTable-full" cellspacing="0" width="100%">
	<thead>
<tr>
<th>S.No.</th><th>Task</th><th>Status</th><th>Assigned By</th><th>Assigned To</th><th>Created Date</th><th>Start Date</th><th>Due Date</th><th>Days Left</th><th>Store Name</th><th>Action</th>
</tr>
</thead>
<tbody>
<?php 
...
//print_r($rrr); 
	// $rrr = DB::table('tasks')->where('createdby',Auth::user()->id)
	// ->whereRaw('id', DB::raw('SELECT taskid FROM `taskissues` where assignfrom='.Auth::user()->id.' OR assignto='.Auth::user()->id.' ORDER BY `tasks`.`id` DESC'))
	//  ->get();
