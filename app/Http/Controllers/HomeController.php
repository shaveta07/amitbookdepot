...
class HomeController extends Controller
{
    ...
    public function otplogin(Request $request)
    {
      ...
      $mobile_no = '+'.$code.$phone; 
        $user = User::whereIn('user_type', ['customer', 'seller'])->where('phone', $mobile_no)->first();
        if($user != null)
        {
            ...
            if (\App\Addon::where('unique_identifier', 'otp_system')->first() != null && \App\Addon::where('unique_identifier', 'otp_system')->first()->activated){
                ...
                if($update_otp)
                {
                    $updated_user = User::where('phone', $mobile_no)->first();
                    sendSMS($updated_user->phone, env('APP_NAME'), $updated_user->verification_code.' is your verification code for '.env('APP_NAME'));
                    $data=array('status'=>"true", 'message'=> 'Otp Send on your Phone Number', 'phone'=>$updated_user->phone, 'otp'=>$updated_user->verification_code);
          ...
          $data=array('status'=>"false", 'message'=> 'User does not exists.', 'phone'=>'', 'otp'=>'');
        return json_encode($data);
    }
    ...
    public function otpverify(Request $request)
    {
        ...
        $user = User::where('phone', $request->otpmob)->first();
        if($user)
        {
            if ($user->verification_code == $request->otp) {
                ...
                // $data=array('status'=>"true", 'message'=> 'Your phone number has been verified successfully');
                // return json_encode($data);
    ...
    public function resend_code(Request $request){
        ...
        $otp = mt_rand(10000,99999);
                ...
                if($update_otp)
                {
                    $updated_user = User::where('phone', $request->otpmob)->first();
                    sendSMS($updated_user->phone, env('APP_NAME'), $updated_user->verification_code.' is your verification code for '.env('APP_NAME'));
    ...
    public function product(Request $request, $slug)
    {
        ...
        if($detailedProduct!=null && $detailedProduct->published){
            ...
            $userData = Auth::user();
            $options = ProductStock::where('product_id', $detailedProduct->id)->get();

            $detailedProduct->bulks = [];
            $detailedProduct->bulks = ProductBulks::where('product_id', $detailedProduct->id)->get();
            
            if($detailedProduct->digital == 1){
                return view('frontend.digital_product_details', compact('detailedProduct','userData'));
            }
            else {
                return view('frontend.product_details', compact('detailedProduct','userData'));
            }
    ...
    public function selected_variation_row(Request $request)
    {
        print_r($request); die;
    }
    ...
    public function variant_price(Request $request)
    {
        ...
        if($str != null && $product->variant_product){
            ...
            $image = $product_stock->variant_image;
            $image_type = "variant_image";
            if($image == NULL)
            {
               $image = $product->photos;
                $image_type = "main_image";
                //$thumb_img = $product->thumbnail_img;
            }
            //$thumb_img =  $product_stock->variant_image;
        ...
        $product_bulks = ProductBulks::where('product_stock_id', $product_stock->id)->orderBy('qtyrange', 'asc')->where('customertype', $bulk_type)->get();
        // return json_encode($product_bulks);
        // if($selected_product_bulk)
        // {
        //     $price = $request->quantity * $selected_product_bulk->overideprice;
        // }
        $bulk_id = [];

        foreach ($product_bulks as $bulk)
        {
            // array_push($bulk_id, $bulk->id);

            $item = [
                "variant" =>  $product_stock->variant,
                "id"    =>  $bulk->id,
                "qty"   =>  $bulk->qtyrange,
                "price" =>  $bulk->overideprice
            ];

            array_push($bulk_id, $item);
        ...
        return array('price' => single_price($price*$request->quantity), 'quantity' => $quantity, 'digital' => $product->digital, 'bulk_id'=>$bulk_id,'image_type'=>$image_type, 'image' => $image);
    }
