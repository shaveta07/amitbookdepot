...
if (! function_exists('sendSMS')) {
    function sendSMS($to, $from, $text)
    {
        ...
        elseif (OtpConfiguration::where('type', 'fast2sms')->first()->value == 1) {
            ...
            return $response;
        }elseif (OtpConfiguration::where('type', 'nicesms')->first()->value == 1) {
            $user = env('NICE_SMS_USER'); // Your Account SID from www.twilio.com/console
            $sid = env('NICE_SMS_SID'); // Your Auth Token from www.twilio.com/console
            ...
            try {
                ...
                $dlr_url="";
                $to = substr($to,3);
                $url = "$api_url?user=$user&authkey=$sid&sender=AMITBK&mobile=$to&text=$message&rpt=0";
